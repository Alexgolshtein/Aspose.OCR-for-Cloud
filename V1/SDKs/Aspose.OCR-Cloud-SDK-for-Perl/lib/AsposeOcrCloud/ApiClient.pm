...
sub new
{
	...
	if(not defined $AsposeOcrCloud::Configuration::app_sid or $AsposeOcrCloud::Configuration::app_sid eq ''){
		croak("Aspose Cloud App SID key is missing.");
    }
	
	if(not defined $AsposeOcrCloud::Configuration::api_key or $AsposeOcrCloud::Configuration::api_key eq ''){
    ...
    my (%args) = (
        ...
        'base_url' => $AsposeOcrCloud::Configuration::api_server,
        @_
    );
...
sub call_api {
	...
	$resource_path =~ s/\Q{appSid}\E/$AsposeOcrCloud::Configuration::app_sid/g;
  
    my $_url = $self->{base_url} . $resource_path;
  
    # update signature
    my $signature = $self->sign($_url, $AsposeOcrCloud::Configuration::app_sid, $AsposeOcrCloud::Configuration::api_key);
    ...
    $self->{ua}->timeout($self->{http_timeout} || $AsposeOcrCloud::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $AsposeOcrCloud::Configuration::http_user_agent);
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("AsposeOcrCloud::Object::$class")->new;
        if (ref $data eq "HASH") {
